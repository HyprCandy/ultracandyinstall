...
trigger_matugen() {
    if [ -f "$MATUGEN_CONFIG" ]; then
        ...
        matugen image "$HOME/.config/wallpaper.png" --type scheme-content -m dark --base16-backend wal --lightness-dark -0.1 --source-color-index 0 -r nearest --contrast 0.15 &
        sleep 3
...
input-field {
    ...
    outer_color = $primary_fixed_dim $surface_bright 90deg
    ...
    numlock_color = $primary_fixed_dim $surface_bright 90deg
...
image {
    ...
    border_size = 4
    border_color = $primary_fixed_dim $surface_bright 90deg
